(function () {
    ...
    let EVENTS = {
        ...
        //IM SDK not ready 事件（退出、被踢出），不可发送消息，不可调用接口（），如想恢复，则调用login接口
        IM_SDK_NOT_READY: "IM_SDK_NOT_READY",
        //收到消息 事件（e.data）
        ON_RECEIVED_MESSAGE: "ON_RECEIVED_MESSAGE",
        //消息已读事件
        ON_MESSAGE_READ: "ON_MESSAGE_READ",
        //消息修改事件
        ON_MESSAGE_MODIFY: "ON_MESSAGE_MODIFY",
        //收到echo事件（e.data）
        ON_RECEIVED_ECHO: "ON_RECEIVED_ECHO",
        //出现 错误 事件(e.data)
        ON_ERROR: "ON_ERROR",
        //网络状态发生变化(e.data) 1网络正常，2网络断开sdk重连中（可在顶部提示 "连接中"）
        NET_STATE_CHANGE: "NET_STATE_CHANGE",
        //收到 被踢出 事件 (e.data)
        ON_KICK_OUT_EVENT: "ON_KICK_OUT_EVENT",
        //会话列表 更新 事件 (e.data)
        ON_SESSION_UPDATE: "ON_SESSION_UPDATE",
        //会话列表总未读数更新 事件 (e.data)
        ON_SESSION_UNREAD_TOTAL_UPDATE: "ON_SESSION_UNREAD_TOTAL_UPDATE",
        //事件同步开始（可在顶部显示 "收取中..."、"同步中..."）
        ON_EVENT_SYNC_START: "ON_EVENT_SYNC_START",
    ...
    let Cl = "undefined" != typeof global ? global : "undefined" != typeof self ? self : "undefined" != typeof window
        ? window : {};

    let Ll = "undefined" != typeof console ? console : void 0 !== Cl && Cl.console ?
        Cl.console : "undefined" != typeof window && window.console ? window.console : {};
                ...
                if (!isNull(session)) {
                    ...
                    if (isNull(split) || isNull(split.length) || split.length !== 2) {
                        ...
                        doRequest({
                            ...
                            body: JSON.stringify({toUser: split[0] === User.id ? split[1] : split[0]}),
                            success(e) {
    ...
    let Message = {
        ...
        receivedMessage(msg) {
            if (!isNull(msg) && !isNull(msg.msgType)) {
                if (msg.msgType === 101) {
                    //消息已读
                    if (!isNull(msg.data) && !isNull(msg.data.msgIds)
                        && msg.data.msgIds.length > 0 && User.id !== msg.data.readUser) {
                    ...
                    if (!isNull(sessionMsgList)) {
                        if (sessionMsgList.length > 0) {
                            ...
                            for (let i = sessionMsgList.length - 1; i >= 0; i--) {
                                if (!isTrue && sessionMsgList[i].msgId === op.nextMsgId || op.nextMsgId === 0) {
                                    isTrue = true;
                                }

                                if (isTrue) {
                                    res.data.nextMsgId = sessionMsgList[i].msgId;
        ...
        start() {
            if (isNull(KeepAlive.tid)) {
                ...
                KeepAlive.tid = setInterval(() => {
                    ...
                    if (!isNull(KeepAlive.lastServerKeepAliveTime)
                        && ((new Date().getTime() - KeepAlive.lastServerKeepAliveTime) > 20000)) {

                        log.debug("long time not receiver server keep alive, last receiver time ："
