(function () {
  ...
  let CONST = {
    gateway: 'http://192.168.88.207:5674',
    // gateway: "http://192.168.88.51:20001/im",
    isSdkReady: false,
  };
    ...
    setLastMsg(msg) {
      if (msg) {
        ...
        if (session != null) {
          ...
          //会话不存在，调用接口创建会话
          //检查sessId正确性
          let sessId = Conversation.getSessId(msg.fromUser, msg.toUser);
              ...
              if (!isNull(e.data) && e.data.length > 0) {
                for (let i = 0; i < e.data.length; i++) {
                  ...
                  let session = Conversation.sessionListMap.get(
                    Conversation.getSessId(cs.id, User.id),
                  );
                  if (!isNull(session)) {
          ...
          if (!isNull(sessionMsgList)) {
            if (sessionMsgList.length > 0) {
              ...
              for (let i = sessionMsgList.length - 1; i >= 0; i--) {
                if (
                  (!isTrue && sessionMsgList[i].msgId === op.nextMsgId) ||
                  op.nextMsgId === 0
                ) {
                  isTrue = true;
                }
  ...
  let MsgSender = {
    ...
    messageSendAndQos(msg, resolve, reject) {
      ...
      if (isNull(msg.msgType)) {
        log.error(
          'error  msgType 不能为空  --->  '.concat(JSON.stringify(msg)),
        );
        reject({ code: -1, message: 'msgType 消息类型 不能为空' });
        return;
      }

      msg.fromUser = User.id;
      msg.fromUserType = User.userType;
      msg.fromUserAvatar = User.avatar;
      ...
      // 填充会话id
      msg.sessId = Conversation.getSessId(msg.fromUser, msg.toUser);
    ...
    notifyFailedMsg(failedMsg) {
      for (let i = 0; i < failedMsg.length; i++) {
        let observer = MsgSender.senderObserver.get(failedMsg[i].msg.cliMsgId);
        if (!isNull(observer)) {
  ...
  let LS = {
    ws: null,
    t: null,
    url: 'ws://192.168.88.207:9193/websocket',
  ...
  return (IMSDK = {
    ...
    /**
     * 创建文本消息
     * @param  op :{content, toUser, toUserType}
     * @return {} textMsg
     */
    createTextMessage(op) {
