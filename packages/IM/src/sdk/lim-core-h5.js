(function () {
    ...
    let Cl = "undefined" != typeof global ? global : "undefined" != typeof self ? self : "undefined" != typeof window ? window : {};

    let Ll = "undefined" != typeof console ? console : void 0 !== Cl && Cl.console ? Cl.console : "undefined" != typeof window && window.console ? window.console : {};
    ...
    function getModelOfBrowser() {
        ...
        function testVs(regexp) {
            return (ua.match(regexp) + "").replace(/[^0-9|_.]/ig, "").replace(/_/ig, ".");
        }
...
//外壳+外壳版本
        ...
        } else if (testUa(/Edg/ig)) {
            shell = "Edg";
            shellVs = testVs(/Edg [\d.]+/ig);
        }
    ...
    let AutoReLogin = {
        ...
        execute: false, tId: null, destroy: false,
            ...
            if (isNull(MsgReceiver.tid)) {
                ...
                MsgReceiver.tid = setInterval(() => {
                    ...
                    if (!MsgReceiver.execute) {
                        try {
                            log.log("[IM][RUNNING] 消息去重保障机制正在运行中... 当前需要处理的队列大小为：" + MsgReceiver.receives.size);
...
// 登录成功   自动重连关闭、心跳启动、发送qos启动、接收qos启动、回调sdk ready、设置loginState
    ...
    let Conversation = {
        ...
        //设置会话
        addSession(session) {
            let tempSession = Conversation.sessionListMap.get(session.sessId);
            if (isNull(tempSession)) {
                Conversation.sessionList.push(session);
                Conversation.sessionListMap.set(session.sessId, session);
        ...
        notifyTotalUnreadCountHandler() {
            let sessUpdateHandler = getEventHandler(EVENTS.ON_SESSION_UNREAD_TOTAL_UPDATE);
            if (!isNull(sessUpdateHandler)) {
        ...
        setLastMsg(msg) {
            if (msg) {
                ...
                if (session != null) {
                    ...
                    //会话不存在，调用接口创建会话
                    //检查sessId正确性
                    let sessId = Conversation.getSessId(msg.fromUser, msg.fromUserType, msg.toUser, msg.toUserType);
                    if (sessId === msg.sessId) {
                        ...
                        promise.then((e) => {
                            ...
                            log.error("获取会话失败，sessId=" + sessId + "  errmsg:" + e.message);
                        })
        ...
        setLastMsgAndIncrUnreadCount(session, msg) {
            session.lastMsg = msg;
            session.unreadCount = isNull(session.unreadCount) ? 0 : session.unreadCount;
            ...
            return new Promise((resolve, reject) => {
                ...
                if (!isNull(session)) {
                    ...
                    if (isNull(split) || isNull(split.length) || split.length !== 2) {
                        ...
                        toUserL = split[0] === User.id ? split[1] : split[0];
                    }
                    doRequest({
                        method: "POST",
                        url: CONST.gateway + "/session/get",
                        body: JSON.stringify({toUser: toUserL}),
                        success(e) {
         ...
         */, fetchNickInfo() {
            if (Conversation.sessionList.length > 0) {
                ...
                doRequest({
                    ...
                    method: "POST", url: CONST.gateway + "/user/infos", body: JSON.stringify({users: users}),
                ...
                if (isNull(op) || isNull(op.sessId)) {
                    ...
                    if (isNull(session)) {
                        ...
                        if (!isNull(session.lastMsg) && !isNull(session.lastMsg.cliMsgId)) {
                            Storage.setSessionUnreadMsgId(session.sessId, session.lastMsg.cliMsgId);
                        }
    ...
    let Message = {
        ...
        receivedMessage(msg) {
            if (!isNull(msg) && !isNull(msg.msgType)) {
                if (msg.msgType === 101) {
                    //消息已读
                    if (!isNull(msg.data) && !isNull(msg.data.msgIds) && msg.data.msgIds.length > 0 && User.id !== msg.data.readUser) {
                        ...
                        for (let j = 0; j < msg.data.msgIds.length; j++) {
                            ...
                            if (!isNull(historyMsg)) {
                                ...
                                historyMsg.readUsers.push({
                                    userId: msg.data.readerUser, userType: msg.data.readerUserType
                                });
                            }
                        }
                        //TODO 修改未读为0、总未读数
        ...
        getMessageList(op) {
            return new Promise((resolve, reject) => {
                if (!isNull(op) && !isNull(op.sessId)) {
                    if (isNull(op.nextMsgId) || op.nextMsgId < 0) {
                        op.nextMsgId = 0;
                    }
                    let res = {
                        data: {
                            isCompleted: false, //消息是否全部拉取完
                    ...
                    if (!isNull(sessionMsgList)) {
                        if (sessionMsgList.length > 0) {
                            ...
                            for (let i = sessionMsgList.length - 1; i >= 0; i--) {
                                if (i === 0) {
                                    res.data.isCompleted = true;
                                }
                                if (res.data.messageList.length >= 20) {
                                    break;
                                }
                                let cliMsgId = sessionMsgList[i].cliMsgId;
                                if (op.nextMsgId === 0) {
                                    //第一次
                                    res.data.nextMsgId = cliMsgId;
                                    //添加到数组第一位
                                    res.data.messageList.unshift(sessionMsgList[i]);
                                } else if (cliMsgId < op.nextMsgId) {
                                    res.data.nextMsgId = cliMsgId;
                                    //添加到数组第一位
                                    res.data.messageList.unshift(sessionMsgList[i]);
                                }
        ...
        createImgMessage(op) {
            return Message.createCustomMessage({
                msgType: 2,//图片消息
                data: op.data, toUser: op.toUser, toUserType: op.toUserType
            })
        },
        ...
        createFileMessage(op) {
            return Message.createCustomMessage({
                msgType: 3, data: op.data, toUser: op.toUser, toUserType: op.toUserType
            })
        },
        ...
        createVideoMessage(op) {
            return Message.createCustomMessage({
                msgType: 4, data: op.data, toUser: op.toUser, toUserType: op.toUserType
            })
        },
        ...
        createPositionMessage(op) {
            return Message.createCustomMessage({
                msgType: 6, data: op.data, toUser: op.toUser, toUserType: op.toUserType
            })
        },
        ...
        doSync() {
            ...
            setTimeout(() => {
                ...
                if (CONST.isSdkReady === false) {
                    ...
                    //拉取会话列表
                    doRequest({
                        method: "POST", url: CONST.gateway + "/session/all", body: "{}", success(e) {
                            if (!isNull(e.code) && e.code === 0) {
                                if (!isNull(e.data) && !isNull(e.data.length) && e.data.length > 0) {
                                    Conversation.sessionList = e.data;
                                    //解析到map中
                                    for (let i = 0; i < e.data.length; i++) {
                                        Conversation.sessionListMap.set(e.data[i].sessId, e.data[i]);
                                    }
                                    //第一次登录拉取昵称头像
            ...
            doRequest({
                ...
                success(e) {
                    ...
                    if (!isNull(e.code) && e.code === 0) {
                        ...
                        if (!isNull(e.data) && !isNull(e.data.data) && e.data.data.length > 0) {
                            ...
                            for (let i = e.data.data.length - 1; i >= 0; i--) {
                                ...
                                if (cData.eventType === 0) {
                                    ...
                                    log.error("接受到新的事件消息，但是当前sdk 无法处理，请升级为最新的sdk")
                                }
                            }
    ...
    let MsgSender = {
        ...
        messageSendAndQos(msg, resolve, reject) {
            ...
            if (isNull(msg.cliMsgId) || msg.cliMsgId <= 0) {
                log.error("error  cliMsgId 不能为空  --->  ".concat(JSON.stringify(msg)));
            ...
            if (isNull(msg.toUserType)) {
                log.error("error  toUserType 不能为空  --->  ".concat(JSON.stringify(msg)));
                reject({code: -1, message: "toUserType 消息接收方类型 不能为空"});
                return;
            }

            if (isNull(msg.msgType) || typeof msg.msgType !== 'number' && isNaN(msg.msgType)) {
                log.error("error  msgType 不能为空，必须是一个 number  --->  ".concat(JSON.stringify(msg)));
                reject({code: -1, message: "msgType 消息类型 不能为空，必须是一个 number"});
                return;
            }

            if (isNull(msg.data) || typeof msg.data !== 'object') {
                log.error("error  data 不能为空，必须是一个 object  --->  ".concat(JSON.stringify(msg)));
                reject({code: -1, message: "data 消息数据 不能为空，必须是一个 object"});
                return;
            }
            ...
            // 填充会话id
            msg.sessId = Conversation.getSessId(msg.fromUser, msg.fromUserType, msg.toUser, msg.toUserType);

            if (isNull(msg.version)) {
                msg.version = IMSDK.version;
            }
            let p = {
                content: msg, id: createUUID(), qos: 1, type: 2
            }

            MsgSender.senderMsg.set(msg.cliMsgId, {
                msg: msg, p: p, lastTime: new Date().getTime(), count: 0, incrCount() {
                    return ++this.count;
                }
            });
    ...
    let KeepAlive = {
        //timer id
        tid: null, lastServerKeepAliveTime: 0, handlerClose: false,
        ...
        start() {
            if (isNull(KeepAlive.tid)) {
                ...
                KeepAlive.tid = setInterval(() => {
                    ...
                    if (!isNull(KeepAlive.lastServerKeepAliveTime) && ((new Date().getTime() - KeepAlive.lastServerKeepAliveTime) > 20000)) {

                        log.debug("long time not receiver server keep alive, last receiver time ：" + KeepAlive.lastServerKeepAliveTime);
        ...
        sendKeep() {
            let p = {
                content: {}, id: createUUID(), qos: 0, type: 4
            }
            let send = LS.send(JSON.stringify(p));
        },
        ...
        doUploadFile(file, progress) {
            return new Promise((resolve, reject) => {
                if (file instanceof File) {
                    log.log("Common.doUploadFile 准备上传文件".concat("  fileName: ").concat(file.name))
                    ...
                    request.ontimeout = () => {
                        log.log("Common.doUploadFile 上传文件失败，网络超时！".concat("  fileName: ").concat(file.name))
                        reject({
                            code: -1, message: "网络超时"
                        });
                    }
                    request.onerror = function (XMLHttpRequest, textStatus, errorThrown) {
                        log.log("Common.doUploadFile 上传文件失败，网络错误！".concat("  fileName: ").concat(file.name))
                        reject({
                            code: -1, message: "网络错误"
                        });
                    }
                    request.onreadystatechange = function () {
                        if (this.readyState === 4 && this.status === 200) {
                            log.log("Common.doUploadFile 上传文件成功 - --->  ok!".concat("  fileName: ").concat(file.name))
    ...
    let IMGroup = {
        /**
         * 创建群聊
         * @param nickName 群名称（必填）
         * @param avatar 群头像（可不填，不填就没有头像）
         * @param groupAddType 群组审批类型，默认为3
         * 1：不允许加入
         * 2：任何人都可以加入
         * 3：群主或管理员审核通过后加入
         * @param members 可在创建群的时候选择群成员，一次最多选择50人，最少1人
         * 数据结构：[{
         *      appUser:"uid",   //必填
         *      role:"群成员角色" //可不填
         * }]
         ...
         * @return Promise
         */
        createGroup(nickName, avatar, groupAddType, members) {
            return new Promise((resolve, reject) => {
                let body = {nickName: nickName, avatar: avatar, groupAddType: groupAddType, appUsers: members}
                doRequest({
                    method: "POST", url: CONST.gateway + "/group/create", body: JSON.stringify(body),
                    success(e) {
                        if (!isNull(e.code) && e.code === 0) {
                            Conversation.addSession(e.data);
                            Conversation.notifySessionUpdate();
                            resolve(e);
                        }
                        reject(e);
        ...
        /**
         * 获取群成员
         * @param groupId 群id（UserType=15）
         * @param nextSeq 分页拉取标志，第一次拉取填0，回调成功如果 nextSeq 不为-1，需要分页，传入再次拉取，直至为-1。
         * return Promise
         */
        getGroupMemberList(groupId, nextSeq) {
            return new Promise((resolve, reject) => {
                if (isNull(groupId)) {
                    reject({code: -1, Message: "群id错误"})
                } else {
                    doRequest({
                        method: "GET",
                        url: CONST.gateway + "/group/member/" + groupId + '?page=' + nextSeq + '&size=30',
                        success(e) {
                            if (!isNull(e.code) && e.code === 0) {
                                resolve(e.data);
                            }
        ...
        addGroupMember(groupId, userIds) {
            return new Promise((resolve, reject) => {
                if (isNull(groupId) || isNull(userIds)) {
                    reject({code: -1, Message: "群id或用户id错误"})
                } else {
                    doRequest({
                        method: "POST",
                        url: CONST.gateway + "/group/invite/join",
                        body: JSON.stringify({
                            groupId: groupId,
                            userIds: userIds
                        }),
        ...
        /**
         * 过滤已经存在该群的成员，返回不在该群的用户appUser和UserId
         * @param groupId 群id
         * @param appUsers 用户appUsers（应用userId） 格式：['appUser','appUser','appUser']
         * return Promise
         */
        filterGroupMemberByAppUser(groupId, appUsers) {
            return new Promise((resolve, reject) => {
                if (isNull(groupId) || isNull(appUsers)) {
                    reject({code: -1, Message: "群id或用户id错误"})
                } else {
                    doRequest({
                        method: "POST",
                        url: CONST.gateway + "/group/member/filter/available",
                        body: JSON.stringify({
                            groupId: groupId,
                            appUsers: appUsers
                        }),
    ...
    let doRequest = (op) => {
        ...
        request.ontimeout = () => {
            ...
            if (log.isDebugLevel()) {
                log.debug("Request Error Common.doRequest.url= ".concat(op.url).concat("  网络超过30秒未正常返回"))
            }
        }
        request.onerror = function (XMLHttpRequest, textStatus, errorThrown) {
            op.fail({
        ...
        request.onreadystatechange = function () {
            if (this.readyState === 4 && this.status === 200) {
                ...
                if (log.isDebugLevel()) {
                    log.debug("Request Success Common.doRequest.url= ".concat(op.url).concat("  res= ").concat(this.responseText))
                }
            }
        };
    ...
    let doLogin = (options) => {
        ...
        return new Promise((resolve, reject) => {
            ...
            log.debug("change loginState =[" + loginState + "]")
            doRequest({
                method: "POST", url: CONST.gateway.concat("/auth/login"), body: JSON.stringify({
                    deviceId: Storage.getDeviceId(), model: Model.getInfo(),
                }), success: (res) => {
                    loginState = 2;
                    log.debug("change loginState =[" + loginState + "]")
    ...
    let doLogout = () => {
        return new Promise((resolve, reject) => {
            doDestroy();
            doRequest({
                method: "GET", url: CONST.gateway.concat("/auth/logout"), body: "", success: (res) => {
                    resolve(res);
                }, fail: (res) => {
                    reject(res);
                }, setHeader: (request) => {
    ...
    return IMSDK = {
        ...
        getSelfUserSync() {
            if (CONST.isSdkReady === true) {
                return {
                    userId: User.id, appUser: User.appUser, nickname: User.nickname, avatar: User.avatar,
                }
            }
            return {}
        },
        ...
        FileApi: {
             ...
             * 上传文件
             * @param file: 文件
             * @param progress: 上传进度回调函数，直接赋值于：xhr.upload.onprogress
             *
             * @return Promise
             */
        ...
        GroupApi: {
             ...
             * 2：任何人都可以加入
             * 3：群主或管理员审核通过后加入
             * @param members 可在创建群的时候选择群成员，一次最多选择50人
             * 数据结构：[{
             *      appUser:"uid",   //必填
             *      role:"群成员角色" //可不填
             * }]
             ...
             * @return Promise
             */
            createGroup: IMGroup.createGroup,
             ...
             * return Promise
             */
            getGroupMemberList: IMGroup.getGroupMemberList,
             ...
             * return Promise
             */
            addGroupMember: IMGroup.addGroupMember,
             ...
             * return Promise
             */
            filterGroupMemberByAppUser: IMGroup.filterGroupMemberByAppUser,
        }
